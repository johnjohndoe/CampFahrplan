...
//		Log.d(LOG_TAG, "lectureListDay: " + MyApp.lectureListDay);
		...
		if (MyApp.lectureListDay != DateInfo.getIndexOfToday(MyApp.dateList, MyApp.dayChangeHour, MyApp.dayChangeMinute)) return;
		Time now = new Time();
		now.setToNow();
		...
		if (!((((now.hour * 60) + now.minute) < firstLectureStart) && DateInfo.sameDay(now, MyApp.lectureListDay, MyApp.dateList))) {
